...
# without ezdrama step: creates play body from wikisource text directly
#######################################################################
...
#      p.tx<-xml_text(sub)
      ...
      if(length(b)>0){
        ...
        print(logx)
        # case: speaker declaration in stage direction, no other speaker on stage, missing extra line
        ma<-grep("allein",p.tx)
        if(length(ma)>0){
         lp<-0 
        }
      ...
      if(l.tx==l.bi){
       ...
       ezd<-paste0("<sp><speaker>",pb.tx,"</speaker>","<stage>",pi.tx,"</stage>")
       l.tx<-100
       l.bi<-200
       print("R2")
       ezd<-rem.dbl(ezd)
